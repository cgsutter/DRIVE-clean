    ...
    if one_off:
        ...
        if train_flag:
            for t in config.trackers_list:
                call_model_train.train_model(
                    run_tracker=t,
                    tracker_rundetails=tracker_rundetails,
                    wandblog=wandblog,
        ...
        if eval_flag:
            ...
            for t in config.trackers_list:
                ...
                # observation level
                preds = results_predictions_obslevel.make_preds(
                    run_tracker=t,
                    tracker_rundetails=tracker_rundetails,
                    wandblog=wandblog,
                    run_arch=config.arch_set,
                    ...
                    run_dr=config.dr_set,
                    run_aug=config.aug,
                    saveflag=savepredsflag,
                    phaseuse=phaseuse_set,
                    inference_otherdata=inference_otherdata_use,
                )
        ...
        for i in range(
            0, len(dfhyp)
        ):  # 7/1/25 hyptuning starting, running in chunks of 10, may take up to 5-6 days. Typicall could be len(dfhyp) # here!! 7/8 need to train [3,10), and then [13,20). 7/12: run (3,32) all at once, i think it may overload dgx to do multiple eval runs
            ...
            if train_flag:
                for t in config.trackers_list:
                    call_model_train.train_model(
                        ...
                        tracker_rundetails=tracker_rundetails,
                        wandblog=wandblog,
                        run_arch=dfhyp["arch"][i],
                        run_trle=dfhyp["trle"][i],
                        run_ast=dfhyp["ast"][i],
                        run_l2=dfhyp["l2"][i],
                        run_dr=dfhyp["dr"][i],
                        run_aug=dfhyp["aug"][i],
            ...
            if eval_flag:
                ...
                if config.eval_highlevel:
                    ...
                    results_df_of30 = results_summaries.run_results_by_exp(
                        preddfs_input=list_dfpreds,
                        preddfs_desc=list_descs30,
