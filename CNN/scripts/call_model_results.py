...
import model_build
import callbacks
import results_predictions
import results_summaries
import pandas as pd
import wandb
import os

def evaluate_exp_models(trackerslist, tracker_rundetails, oneoff_flag, hyp_flag, dfhyp, dataset_all, all_images, arch_eval, trle_eval, ast_eval, dr_eval, l2_eval): # all_labels
    ...
    if oneoff_flag:
        ...
        for t in trackerslist:
            ...
            df_preds, t_name = results_predictions.evaluate(modeldir = f"{config.model_path}/{tracker_details}", dataset = dataset_all, imgnames = all_images, trackerinput = t, saveto = f"{config.preds_path}/{tracker_details}.csv", saveflag = True, run_arch = arch_eval, run_trle = trle_eval, run_ast = ast_eval, run_dr = dr_eval, run_l2 = l2_eval)
            preddfs_30.append(df_preds)
            descs_30.append(t_name)

        print("one_off evalC")
        for phase in ["", "innerTrain", "innerVal", "innerTest", "outerTest"]:
            results_df_of30 = results_summaries.run_results_by_exp(preddfs_input = preddfs_30, preddfs_desc = descs_30,  exp_desc_input = config.exp_desc, exp_details_input = tracker_rundetails, subsetphase = phase)
    ...
    if hyp_flag: # have two sets of loops for both the trackers and the hyperparameter set
        # for i in range(0,len(trackerslist)): #HERE!! loop by trackerslist
        ...
        for t in config.trackers_list:
            print(f"ENTER4: inside loop by tracker within call_model_results, should be ran the amount of times dependent on # of trackers")
            print("inside loop for evaluation")

            tracker_details = helper_fns_adhoc.prep_filedetails(tracker_designated = t, rundetails = tracker_rundetails)

            df_preds, t_name = results_predictions.evaluate(modeldir = f"{config.model_path}/{tracker_details}", dataset = dataset_all, imgnames = all_images, trackerinput = t, saveto = "", saveflag = False, run_arch = arch_eval, run_trle = trle_eval, run_ast = ast_eval, run_dr = dr_eval, run_l2 = l2_eval)
            preddfs_30.append(df_preds)
            descs_30.append(t_name)
        ...
        results_df_of30 = results_summaries.run_results_by_exp(preddfs_input = preddfs_30, preddfs_desc = descs_30,  exp_desc_input = config.exp_desc, exp_details_input = tracker_rundetails, subsetphase = "innerVal")
