# Source: this is Keras Callback for early stopping, but with edits made to only allow early stopping (e.g. 10 epochs of no improvement) to kick in after a minimum of 30 epochs have ran. For example, may stop after 31 epochs (if no improvement from 21-31), but it won't stop after 11 epochs.
# Keras source code: https://github.com/keras-team/keras/blob/v2.13.1/keras/callbacks.py#L1970
...
from keras.distribute import distributed_file_utils, worker_training_state
from keras.optimizers.schedules import learning_rate_schedule
from keras.utils import generic_utils, io_utils, tf_utils, version_utils
...
def _is_generator_like(data):
    ...
    return (
        ...
        or isinstance(data, (Sequence, tf.compat.v1.data.Iterator, tf.data.Iterator))
    )
        ...
        if self.load_weights_on_restart:
            ...
            if filepath_to_load is not None and self._checkpoint_exists(
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error loading file from {filepath_to_load}. " f"Reason: {e}"
                    )
...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.wait >= self.patience and epoch > config.min_epochs_before_es:  # HERE
...
class LearningRateScheduler(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if not isinstance(lr, (tf.Tensor, float, np.float32, np.float64)):
            raise ValueError(
                'The output of the "schedule" function ' f"should be float. Got: {lr}"
            )
    ...
    * Sampled profiling
    ...
    def _train_writer(self):
        if "train" not in self._writers:
            self._writers["train"] = tf.summary.create_file_writer(self._train_dir)
            ...
            with tf.summary.record_if(True):
                ...
                if hasattr(train_fn, "function_spec"):
                    ...
                    if hasattr(train_fn, "_concrete_stateful_fn"):
                        ...
                        tf.summary.graph(train_fn._concrete_variable_creation_fn.graph)
...
class ReduceLROnPlateau(Callback):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if factor >= 1.0:
            raise ValueError(
                "ReduceLROnPlateau does not support " f"a factor >= 1.0. Got {factor}"
            )
...
class LambdaCallback(Callback):
    ...
    cleanup_callback = LambdaCallback(
        on_train_end=lambda logs: [p.terminate() for p in processes if p.is_alive()]
    )

    model.fit(
        ..., callbacks=[batch_print_callback, json_logging_callback, cleanup_callback]
    )
    ```
    """
