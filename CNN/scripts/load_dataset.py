...
def load_and_ready_image(image_path, arch_str, aug_bool):
    ...
    img_processed = preprocess_and_aug(
        image_np=image_array,
        arch_for_preprocess=arch_str_format,
        augflag=aug_bool_format,
    )
    ...
    return (
        img_processed.numpy()
    )  # return numpy object so that when called on in tf.py_function, which automatically converts it to a tf tensor, it won't already be a tf object (which can cause problems)


def tf_load_and_preprocess_with_label(
    image_path, label, arch_for_preprocess_tensor, augflag_tensor
):
    ...
    processed_image = tf.py_function(
        ...
        inp=[
            image_path,
            arch_for_preprocess_tensor,
            augflag_tensor,
        ],  # Pass all four tf.Tensor inputs
...
def load_data(trackerinput, phaseinput, archinput, auginput):
    ...
    print(trackerinput)
    print(phaseinput)
    all_evaluation_image_paths, all_evaluation_labels, catcounts, imgnames = (
        grab_img_paths_labels(tracker=trackerinput, phase=phaseinput)
    )

    numims = len(all_evaluation_image_paths)

    print(
        f"Found {len(all_evaluation_image_paths)} images with corresponding labels for evaluation."
    )

    # Create a dataset from image paths AND labels
    path_and_label_dataset = tf.data.Dataset.from_tensor_slices(
        (all_evaluation_image_paths, all_evaluation_labels)
    )

    # --- Define the specific arch and augflag values for THIS pipeline run ---
    # These must be tf.Tensor constants
    arch_tensor_constant = tf.constant(
        archinput, dtype=tf.string
    )  # Example architecture
    augflag_tensor_constant = tf.constant(
        auginput, dtype=tf.bool
    )  # For evaluation, typically False
