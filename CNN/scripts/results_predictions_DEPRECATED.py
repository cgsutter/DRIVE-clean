...
def evaluate(modeldir, dataset, imgnames, trackerinput, saveflag = False, saveto = "", run_arch = config.arch_set,run_trle = config.transfer_learning, run_ast = config.ast, run_dr = config.dr_set, run_l2 = config.l2_set ):
    ...
    print(f"DEBUG: Starting detailed batch inspection for iteration in loop") # 'i' from your main loop
    ...
    try:
        for batch_idx, batch_data in enumerate(dataset_for_prediction):
            print(f"  Inspecting batch {batch_idx} (Iteration). Shape: {batch_data.shape}, Dtype: {batch_data.dtype}")
            
            # Check for problematic batch shapes:
            if not tf.is_tensor(batch_data):
                print(f"    ERROR: Batch {batch_idx} is not a tensor! Type: {type(batch_data)}")
                raise ValueError("Batch data is not a tensor (Non-Tensor Batch).")
            
            if batch_data.shape.rank == 0: # Scalar batch
                print(f"    ERROR: Batch {batch_idx} is a scalar! Shape: {batch_data.shape}, Value: {batch_data.numpy()}")
                raise ValueError("Batch data is a scalar (Scalar Batch).")
            
            if batch_data.shape[0] == 0: # Empty batch
                print(f"    WARNING: Batch {batch_idx} has a batch size of 0. This might be problematic for model.predict.")
            ...
            try:
                ...
                print(f"    ERROR: model.predict_on_batch failed for batch {batch_idx} (Iteration). Error: {batch_e}")
        ...
        print(f"DEBUG: All batches successfully processed via predict_on_batch for iteration.")
        ...
        p2 = model.predict(dataset_for_prediction) # This is where the original error happens

    except Exception as full_dataset_iter_e:
        print(f"ERROR: An exception occurred during full dataset iteration (Iteration): {full_dataset_iter_e}")
        print("This indicates an issue with a specific batch within the dataset, or a dataset pipeline issue not caught by individual predict_on_batch calls.")
