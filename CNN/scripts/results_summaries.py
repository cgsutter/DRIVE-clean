...
def identify_ok_preds(ytrue, ypred):
    ...
    for i in range(0, len(ytrue)):
        if ytrue[i] == ypred[i]:
            ok = True
        elif (ytrue[i] == "snow_severe") & (ypred[i] == "snow"):
            ok = True
        elif (ytrue[i] == "snow") & ((ypred[i] == "snow_severe") | (ypred[i] == "wet")):
            ok = True
        elif (ytrue[i] == "wet") & ((ypred[i] == "snow") | (ypred[i] == "dry")):
            ok = True
        elif (ytrue[i] == "dry") & (ypred[i] == "wet"):
            ok = True
        else:
            ok = False
        oks.append(ok)
...
def calc_summary(dfinput="", tracker_desc="", exp_desc=config.exp_desc, exp_details=""):
    ...
    metrics_list, results_list = calcstats_onefold(
        ytrueinput=dfinput["img_cat"], ypredinput=dfinput["model_pred"]
    )
    # print(results_dict)
...
def run_results_by_exp(
    ...
    for df_i, desc_i in zip(preddfs_input, preddfs_desc):
        if (
            (subsetphase == "innerTrain")
            | (subsetphase == "innerVal")
            | (subsetphase == "innerTest")
        ):
        ...
        dict_i = calc_summary(
            dfinput=df_i,
            tracker_desc=desc_i,
            exp_desc=exp_desc_input,
            exp_details=exp_details_input,
        )
...
def exp_total_innerVal(
    df_innerVal, exp_desc_input=config.exp_desc, exp_details_input=""
):
    """
