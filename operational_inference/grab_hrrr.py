...
def format_df_for_mapping_hrrr(modelpreds, fcsthr_num=2):
    ...
    # THEN GET INIT TIME FROM THAT. THIS IS THE DATE TIME THAT SHOULD BE USED TO FIND THE RIGHT HRRR FILE, STARTING WITH DIRECTORY PATH, AND THEN ALSO THE FILE NAME
    ...
    modelpreds["init_year"] = modelpreds["date_and_time_dt_init"].dt.strftime("%Y")
    modelpreds["init_month"] = modelpreds["date_and_time_dt_init"].dt.strftime("%m")
    modelpreds["init_day"] = modelpreds["date_and_time_dt_init"].dt.strftime("%d")
    modelpreds["init_yyyymmdd"] = modelpreds["date_and_time_dt_init"].dt.strftime(
        "%Y%m%d"
    )
    ...
    # modelpreds.head(6)
    # not using this any where
    # modelpreds["yyyy"] = modelpreds.yr.apply(str)

    # filename_without fcst hour (will pull 3 different init times)

    # UPDATED 12312024: needed to look for init yr/mo/day for dir path, NOT the valid time
    pre_filename = (
        f"/home/csutter/AI2ES/cleaned/HRRR/"
        + modelpreds.init_year.apply(str)
        + "/"
        + modelpreds.init_month.apply(str).str.zfill(2)
        + "/"
        + modelpreds.init_yyyymmdd.apply(str)
        + "_hrrr.t"
    )
    ...
    #     pre_filename + modelpreds.init_hr.apply(str) + "z_02.parquet"
    # ) # was hardcoded 02 here

    modelpreds["hrrr_file"] = (
        pre_filename + modelpreds.init_hr.apply(str) + f"z_{fcsthr_str_2dig}.parquet"
    )
...
##### READ IN DATA AND EVALUATE FNS ABOVE
...
def runall(fhnum_input, imgdatacsv, hrrrdatacsv_tosave):
    ...
    # identify the right HRRR file name to use for each observation
    df2 = format_df_for_mapping_hrrr(imgcamdata, fcsthr_num=fhnum_input)
    

    hrrrdatalists = Parallel(n_jobs=cores_to_use)(
        delayed(colocate_add_hrrr_data)(row) for index, row in df2.iterrows()
    )  # down to  ~1 min using 192 CPU (75% of 256 cores)
    print("DONE WITH COLOCATING!!!")
    # print(len(hrrrdatalists))
    # print(hrrrdatalists)
    # print(type(hrrrdatalists))

    # # example of the 0th observation
    # # print(g2[0][0])
    # # print(g2[0][1])
    # # print(g2[0][2])
    ...
    fh_str_onedigit = str(fhnum_input)

    # can save out intermediate step if want to check
    # print(f"/home/csutter/DRIVE/weather_img_concatmodels/cnn_hrrr_fcsthr{fh_str_onedigit}/intermed_LIERC1RC2_imgname_hrrrdata_fcsthr{fh_str_onedigit}.csv")
    # df2.to_csv(
    #     f"/home/csutter/DRIVE/weather_img_concatmodels/cnn_hrrr_fcsthr{fh_str_onedigit}/intermed_LIERC1RC2_imgname_hrrrdata_fcsthr{fh_str_onedigit}.csv"
    # )
    # print("successfully saved")
    # print("through df2 here")
    # print(df2.head(4))
    ...
    print(hrrrdatacsv_tosave)

    # print(f"{parentdir}/step2_imgandhrrr.csv")
    # Save out as step2
    df_obs.to_csv(hrrrdatacsv_tosave)

# See /home/csutter/DRIVE/weather_img_concatmodels/cnn_hrrr_fcsthr2/hrrr_colocate_fcsthr2.py for other forecast hours
